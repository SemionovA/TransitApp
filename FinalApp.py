...
the COVID-19 outbreak, with the exception of the local funding for KCM that decreases significantly after 2015.\n\n\n *All results can be found in the report\
 with the according visuals.*')
    ...
    with st.sidebar:
        ...
        st.text("")
        agency_select0 = st.selectbox('Select Agency', agency_choice, key=2)
        year_select0 = st.selectbox('Select Year', year_choice, key=3)
        st.text("")
        compare = st.checkbox('Compare to another year or agency?', value=False)
        st.markdown("***")
        st.text("")
    if compare == True:
        with st.sidebar:
            disableAgencySelect1 = False
            agency_select1 = st.selectbox('Select Agency #2', agency_choice, key=4, disabled = disableAgencySelect1)
            same_agency = st.checkbox('Use same agency as above', value=False)
            if same_agency == True:
                disableAgencySelect1 = True
                agency_select1 = agency_select0
    
            disableYearSelect1 = False
            year_select1 = st.selectbox('Select Year #2', year_choice, key=5, disabled = disableYearSelect1)
            same_year = st.checkbox('Use same year as above', value=False)
            if same_year == True:
                disableYearSelect1 = True
                year_select1 = year_select0
    ...
    if data_select == 'Funding':
        fed_select0 = fed.at[agency_select0, year_select0]
        state_select0 = state.at[agency_select0, year_select0]
        local_select0 = local.at[agency_select0, year_select0]
        op_select0 = op.at[agency_select0, year_select0]
        fares_select0 = fares.at[agency_select0, year_select0]
        dollar_amounts0 = [fed_select0, state_select0, local_select0, op_select0, fares_select0]
        categories0 = [
        f'Federal Funding - ${"{:,}".format(int(fed_select0))}',
        f'State Funding - ${"{:,}".format(int(state_select0))}',
        f'Local Funding - ${"{:,}".format(int(local_select0))}',
        f'Other Operational Funding - ${"{:,}".format(int(op_select0))}',
        f'Fare Revenue - ${"{:,}".format(int(fares_select0))}']
        #>>> Extract expenditure values for given year, agency
    elif data_select == 'Expenditure':
        capex_select0 = capex.at[agency_select0, year_select0]
        opex_select0 = opex.at[agency_select0, year_select0]
        dollar_amounts0 = [capex_select0, opex_select0]
        categories0 = [
        f'Capital Expenses - ${"{:,}".format(int(capex_select0))}',
        f'Operating Expenses - ${"{:,}".format(int(opex_select0))}']
    
    #>> Pie Chart Generation
    fig, ax = plt.subplots()
    ax.pie(dollar_amounts0, labels=categories0, autopct='%1.1f%%',
            shadow=True, startangle=90)
    ax.axis('equal')
    ax.set_title(f'{agency_select0} {year_select0} {data_select} Breakdown', wrap = True)
    ...
    if compare == True:
        ...
        #>>> Extract funding values for given year, agency
        if data_select == 'Funding':
            fed_select1 = fed.at[agency_select1, year_select1]
            state_select1 = state.at[agency_select1, year_select1]
            local_select1 = local.at[agency_select1, year_select1]
            op_select1 = op.at[agency_select1, year_select1]
            fares_select1 = fares.at[agency_select1, year_select1]
            dollar_amounts1 = [fed_select1, state_select1, local_select1, op_select1, fares_select1]
            categories1 = [
            f'Federal Funding - ${"{:,}".format(int(fed_select1))}',
            f'State Funding - ${"{:,}".format(int(state_select1))}',
            f'Local Funding - ${"{:,}".format(int(local_select1))}',
            f'Other Operational Funding - ${"{:,}".format(int(op_select1))}',
            f'Fare Revenue - ${"{:,}".format(int(fares_select1))}']
            #>>> Extract expenditure values for given year, agency
        elif data_select == 'Expenditure':
            capex_select1 = capex.at[agency_select1, year_select1]
            opex_select1 = opex.at[agency_select1, year_select1]
            dollar_amounts1 = [capex_select1, opex_select1]
            categories1 = [
            f'Capital Expenses - ${"{:,}".format(int(capex_select1))}',
            f'Operating Expenses - ${"{:,}".format(int(opex_select1))}']
    
        #>> Pie Chart Generation
        fig1, ax1 = plt.subplots()
        ax1.pie(dollar_amounts1, labels=categories1, autopct='%1.1f%%',
                shadow=True, startangle=90)
        ax1.axis('equal')
        ax1.set_title(f'Vs. \n\n{agency_select1} {year_select1} {data_select} Breakdown', wrap = True)
